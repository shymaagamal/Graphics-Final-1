...
//	1.2.0	20020130	PRL		Add new token 'texture' to apply textures to individual groups.
//								glmDraw, glmDrawArrays and glmList are now OpenGL context-specific.
//	1.2.1	20020203	PRL		New glmVertexNormals() routine to better handle models with polys with > 3 sides.
//	1.3.0	20030805	PRL		Added to glmReadMTL/glmWriteMTL to handle materials with texture maps
//								and dissolve factors. glmReadOBJ is now OpenGL context-specific.
//

#include "eden/glm.h"
#ifdef __APPLE__
#  include <OpenGL/glext.h>		// glLockArraysEXT(), glUnlockArraysEXT()
#endif
 ...
 */
static GLvoid
glmReadMTL(GLMmodel* model, char* name, const int contextIndex)
{
    FILE* file;
    char* dir;
    char* filename;
	...
	TEXTURE_INFO_t textureInfo = {		// PRL 20030806: Defaults for texturemaps.
		NULL,							//   pointer to name will go here.
		GL_TRUE,						//   generate mipmaps.
		GL_RGBA8,						//   internal format.
		GL_LINEAR_MIPMAP_NEAREST,		//   minification mode.
		GL_LINEAR,						//   magnification mode.
		GL_REPEAT,						//   wrap_s.
		GL_REPEAT,						//   wrap_t.
		0.5,							//   priority.
		GL_MODULATE,					//   env_mode.
    ...
    while(fscanf(file, "%s", buf) != EOF) {
		...
		case 'm':		// PRL 20030806: texturemap.
			if (strstr(buf, "map_Kd")) {
				...
				textureInfo.pathname = model->materials[nummaterials].texturemap;
				SurfacesTextureLoad(contextIndex, 1, &textureInfo, &(model->materials[nummaterials].texturemap_index));
			} else {
 ...
 */
static GLvoid
glmFirstPass(GLMmodel* model, FILE* file, const int contextIndex) 
{
    ...
    while(fscanf(file, "%s", buf) != EOF) {
		...
		case 'm':				/* material library */
			...
			glmReadMTL(model, buf, contextIndex);
			break;
		case 'u':				/* usemtl */
			/* eat up rest of line */
			fgets(buf, sizeof(buf), file);
...
glmSecondPass(GLMmodel* model, FILE* file) 
{
    ...
    while(fscanf(file, "%s", buf) != EOF) {
		...
		case 'g':               /* group */
			...
			fgets(buf, sizeof(buf), file);		// Read up to (and including) EOL from file into string.
#if SINGLE_STRING_GROUP_NAMES
			sscanf(buf, "%s", buf);				// Keep only up to first whitespace.
			group = glmFindGroup(model, buf);
#else
			buf[strlen(buf)-1] = '\0';  /* nuke '\n' */
			group = glmFindGroup(model, buf+1);	// +1 skips leading space.
#endif
			group->material = material;			// Curent material is default for new group.
			break;
		case 'f':               /* face */
			v = n = t = 0;
			fscanf(file, "%s", buf);
...
glmVertexNormals(GLMmodel* model, GLfloat angle)
{
    ...
    for (i = 1; i <= model->numvertices; i++) {
		...
		node = members[i];
		if (!node)
            fprintf(stderr, "glmVertexNormals(): Model has vertex not contained in any triangle.\n");
		while (node) {
...
glmLinearTexture(GLMmodel* model)
{
    ...
    scalefactor = 2.0 / glmAbs(glmMax(dimensions[0], dimensions[2]));
...
#if 0
    fprintf(stderr,"glmLinearTexture(): generated %d linear texture coordinates\n",
        model->numtexcoords);
#endif
}
    ...
    if (model->materials) {
        for (i = 0; i < model->nummaterials; i++) {
			...
			if (model->materials[i].texturemap_index)		// PRL 20030806: texturemap.
				SurfacesTextureUnload(contextIndex, 1, &model->materials[i].texturemap_index);
...
glmReadOBJ(char* filename, const int contextIndex)
{
    ...
    glmFirstPass(model, file, contextIndex);
...
glmWriteOBJ(GLMmodel* model, char* filename, GLuint mode)
{
    ...
    if (mode & GLM_FLAT && !model->facetnorms) {
        fprintf(stderr,"glmWriteOBJ() warning: flat normal output requested "
            "with no facet normals defined.\n");
        mode &= ~GLM_FLAT;
    }
    if (mode & GLM_SMOOTH && !model->normals) {
        fprintf(stderr,"glmWriteOBJ() warning: smooth normal output requested "
            "with no normals defined.\n");
        mode &= ~GLM_SMOOTH;
    }
    if (mode & GLM_TEXTURE && !model->texcoords) {
        fprintf(stderr,"glmWriteOBJ() warning: texture coordinate output requested "
    ...
    if (mode & GLM_COLOR && !model->materials) {
        fprintf(stderr,"glmWriteOBJ() warning: color output requested "
            "with no colors (materials) defined.\n");
        mode &= ~GLM_COLOR;
    }
    if (mode & GLM_MATERIAL && !model->materials) {
        fprintf(stderr,"glmWriteOBJ() warning: material output requested "
            "with no materials defined.\n");
        mode &= ~GLM_MATERIAL;
    }
    if (mode & GLM_COLOR && mode & GLM_MATERIAL) {
        fprintf(stderr,"glmWriteOBJ() warning: color and material output requested "
    ...
    fprintf(file, "\n");
    fprintf(file, "# %ld groups\n", model->numgroups);
    fprintf(file, "# %ld faces (triangles)\n", model->numtriangles);
    fprintf(file, "\n");
...
glmDraw(GLMmodel* model, GLuint mode, const int contextIndex)
{
	...
	/* do a bit of warning */
	if (mode & GLM_FLAT && !model->facetnorms) {
		fprintf(stderr,"glmDraw() warning: flat render mode requested "
			"with no facet normals defined.\n");
		mode &= ~GLM_FLAT;
	}
	if (mode & GLM_SMOOTH && !model->normals) {
		fprintf(stderr,"glmDraw() warning: smooth render mode requested "
			"with no normals defined.\n");
		mode &= ~GLM_SMOOTH;
	}
	if (mode & GLM_TEXTURE && !model->texcoords) {
		fprintf(stderr,"glmDraw() warning: texture render mode requested "
			"with no texture coordinates defined.\n");
		mode &= ~GLM_TEXTURE;
	}
	if (mode & GLM_FLAT && mode & GLM_SMOOTH) {
	...
	if (mode & GLM_COLOR && !model->materials) {
		fprintf(stderr,"glmDraw() warning: color render mode requested "
			"with no materials defined.\n");
		mode &= ~GLM_COLOR;
	}
	if (mode & GLM_MATERIAL && !model->materials) {
		fprintf(stderr,"glmDraw() warning: material render mode requested "
			"with no materials defined.\n");
		mode &= ~GLM_MATERIAL;
	}
	if (mode & GLM_COLOR && mode & GLM_MATERIAL) {
		fprintf(stderr,"glmDraw() warning: color and material render mode requested "
...
#ifdef GLM_MATERIAL_TEXTURES
			if (mode & GLM_TEXTURE) {			// PRL 20030806: Texture if material has texturemap.
				if (material->texturemap_index) {
					SurfacesTextureSet(contextIndex, material->texturemap_index);
...
#ifdef GLM_MATERIAL_TEXTURES
		if ((mode & GLM_TEXTURE) && material->texturemap_index)	// PRL 20030806: Only look for texcoords if texturemapping.
#else
		if (mode & GLM_TEXTURE)
#endif
		{
...
#if 0
    fprintf(stderr,"glmWeld(): %d redundant vertices.\n", 
 ...
 *
 * Uses model->vertices as the actual data array.
 * Will need some changes before it can actually be used to supply normals and texcoords!
 *
 * model - initialized GLMmodel structure
 * mode  - a bitwise OR of values describing what is to be rendered.
 *             GLM_NONE     -  render with only vertices
...
{
	static GLuint	**indices	= NULL;			// Handle to elements.
	GLuint			i, j;
	GLMtriangle*	triangle;
	GLMgroup*		group;
	GLMmaterial*	material;

	/* Create the interleaved data arrays the first time through. */
	if (!indices) {
		...
		/* do a bit of warning */
		if (mode & GLM_FLAT) {
			fprintf(stderr,"glmDrawArrays() Error: flat render mode requested.\n");
			exit(-1);
		}
		if (mode & GLM_SMOOTH && !model->normals) {
			fprintf(stderr,"glmDrawArrays() warning: smooth render mode requested "
		  "with no normals defined.\n");
			mode &= ~GLM_SMOOTH;
		}
		if (mode & GLM_TEXTURE && !model->texcoords) {
			fprintf(stderr,"glmDrawArrays() warning: texture render mode requested "
		  "with no texture coordinates defined.\n");
			mode &= ~GLM_TEXTURE;
		}
		if (mode & GLM_COLOR && !model->materials) {
			fprintf(stderr,"glmDrawArrays() warning: color render mode requested "
		  "with no materials defined.\n");
			mode &= ~GLM_COLOR;
		}
		if (mode & GLM_MATERIAL && !model->materials) {
			fprintf(stderr,"glmDrawArrays() warning: material render mode requested "
		  "with no materials defined.\n");
			mode &= ~GLM_MATERIAL;
		}
		if (mode & GLM_COLOR && mode & GLM_MATERIAL) {
			fprintf(stderr,"glmDrawArrays() warning: color and material render mode requested "
		  "using only material mode.\n");
			mode &= ~GLM_COLOR;
		}

		// Prepare a vertex index array for each group in the model.
		// Make the indices for each group in the model by copying the vertex indices from the triangles.
		indices = (GLuint **)malloc(sizeof(GLuint *) * model->numgroups);	// Allocate array of pointers to indices.
		group = model->groups;
		for (i = 0; i < model->numgroups; i++) {
			indices[i] = (GLuint *)malloc(sizeof(GLuint) * 3 * group->numtriangles); // Allocate array of indices.
			for (j = 0; j < group->numtriangles; j++) {
				triangle = &T(group->triangles[j]);
				indices[i][j*3    ] = triangle->vindices[0];
				indices[i][j*3 + 1] = triangle->vindices[1];
				indices[i][j*3 + 2] = triangle->vindices[2];
			}
			group = group->next;
		}
	}
	...
	// Save client state before enabling the arrays we are using.
	glPushClientAttrib(GL_CLIENT_VERTEX_ARRAY_BIT);
	//glInterleavedArrays(format, 0, data);		// Only for interleaved data.
...
#endif
	glLockArraysEXT(0, model->numvertices * 3);	// Compile arrays.
	...
	for (i = 0; i < model->numgroups; i++) {
		...
		// Draw the group.
		glDrawElements(GL_TRIANGLES, group->numtriangles * 3, GL_UNSIGNED_INT, indices[i]);

		group = group->next;
	}

	glUnlockArraysEXT();
	glPopClientAttrib();		// Restore client state.
...
if (model->numnormals) {
    ...
    fprintf(stderr,"glmOptimize(): %d redundant normals.\n", 
...
if (model->numtexcoords) {
    ...
    fprintf(stderr,"glmOptimize(): %d redundant texcoords.\n", 
